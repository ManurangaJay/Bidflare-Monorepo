...
public class AuctionMapper {
    ...
    public static Auction toEntity(AuctionCreateDto dto, Product product) {
        return Auction.builder()
                .product(product)
                .startTime(dto.startTime().atZone(ZoneId.systemDefault()))
                .endTime(dto.endTime().atZone(ZoneId.systemDefault()))
                .isClosed(false)
                .build();
    }

    public static void updateAuction(Auction auction, AuctionUpdateDto dto, User winner) {
        if (dto.startTime() != null) auction.setStartTime(dto.startTime().atZone(ZoneId.systemDefault()));
        if (dto.endTime() != null) auction.setEndTime(dto.endTime().atZone(ZoneId.systemDefault()));
